# Ибрагимов Тимур Марселевич БПИ239 Вариант 35
## Текст задания:
>Разработать программу, определяющую корень уравнения x^4−x^3−2.5 = 0 методом хорд с точностью от 0,001 до 0,00000001 в диапазоне [1;2].
>Если диапазон некорректен, то подобрать корректный диапазон.
## Метод решения:
* Для того, чтобы проверить корректность диапазона я построил график заданной функции в `desmos`. Диапазон оказался корректным (один из корней располагается строго внутри орезка), значит проверять его программно нет необходимости.
* Описание метода хорд и необходимая формула были найдены [тут](https://en.wikipedia.org/wiki/Secant_method).
* Программа на каждой итерации сравнивает прошлое приблежение х с текущим, найденным по формуле из статьи выше, и в случае, если абсолютная разница между этими значениями оказывается меньше заданной погрешности, завершает работу и возвращает последний полученный результат.
## 4-5 баллов
